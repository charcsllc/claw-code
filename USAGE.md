...
- `/design-review` — el gate de diseño del multiagente, aplicable a **cualquier proyecto**: contraste WCAG de los tokens, accesibilidad del HTML (audita `index.html` si no hay DOM renderizado) y disciplina de tokens. 0 tokens de IA.
- `/review <rama-base>` — además de `staged`, acepta una referencia git: revisa todo lo que tu rama añade sobre la base (`/review main` antes de una PR).
...
- `/design-review fix` — tras la auditoría determinista, entrega los hallazgos al modelo en un turno para que los corrija (la detección es computada; la reparación, no).
- `/diff [archivo]` — limita el diff a un solo archivo (`/diff src/main.rs`); la ruta viaja tras `--` para que git nunca la lea como flag.
- `/retry` — reenvía el último prompt tal cual (el clásico tras un fallo de red); antes estaba anunciado en `/help` pero sin implementar.
- `/tasks [list|get <id>]` — lista los agentes en background del workspace (manifiestos de `.clawd-agents/`) con estado running/done/failed.
- `/doctor online` — el doctor estático más una petición real de 1 token al proveedor activo y el inventario de variables de credenciales detectadas.
- `/model <fragmento>` — resolución difusa: un fragmento inequívoco de un ID canónico resuelve solo (`/model sonnet-5`); si es ambiguo lista los candidatos; los nombres desconocidos pasan intactos (modelos custom/ollama siguen funcionando).
- `/stats` — desglose acumulado de tokens y coste estimado de la sesión (input, output y caché de prompts).
- `/tokens` y `/cache` — alias registrados de `/stats`: el mismo informe de tokens/coste bajo el nombre que te resulte más natural.
- `/version` — versión del binario en uso y su procedencia, sin salir del REPL.
- `/clear --confirm` — arranca una sesión nueva en limpio conservando modelo y modo de permisos; sin `--confirm` solo avisa, y la sesión anterior queda recuperable con `/resume <id>`.
- `/resume [id|latest]` — cambia a otra sesión guardada sin salir del REPL (la actual se persiste antes de saltar).
- `/sandbox` — informe del sandbox: la política efectiva que resuelve la configuración para el directorio actual.
- `/memory` — la memoria del proyecto que se inyecta en el contexto: qué archivos de instrucciones (CLAUDE.md, CLAW.md, AGENTS.md…) se descubrieron en la jerarquía del cwd, con líneas, tamaño y preview.
- `/init` — inicializa el repositorio actual desde el REPL (el mismo scaffolding que `claw init`).
- `/agents [create <nombre>|list]` — gestiona los agentes locales del workspace (`.claw/agents/*.toml`): lista los definidos o genera el andamiaje de uno nuevo.
- `/pin [texto]` — fija una nota en la sesión (por defecto, tu último prompt); **sobrevive a `/compact`**. `/bookmarks` las lista y `/unpin [n]` las quita.
- `/focus <ruta> [ruta...]` — cada turno siguiente pide priorizar esas rutas sin reescribirlas; `/unfocus` lo quita.
- `/undo` — revierte el último `edit_file` del modelo reaplicando el intercambio inverso; solo actúa si el texto editado sigue intacto (si el archivo cambió después, lo dice en vez de adivinar). Encadenable: cada `/undo` retrocede una edición más.
- `/stop` — cancela los agentes en background en ejecución (también `/tasks stop <id>` para uno concreto); el turno en curso se cancela con Ctrl+C. La cancelación es cooperativa: surte efecto entre pasos.
- `/compact preview` — muestra cuántos mensajes eliminaría y conservaría la compactación **sin tocar la sesión**.
- `/provider models` — consulta el endpoint `/models` real del proveedor activo (timeout 10s) y lista los modelos disponibles de verdad, no los defaults.
- `/context` — ahora añade un **desglose por categoría** (system prompt, prompts, respuestas, tráfico de herramientas) con barras, para saber qué recortar.

Además: el banner de arranque muestra la versión y el proveedor activo (y avisa si no hay credenciales); `/doctor` muestra el proveedor, el timeout de bash efectivo y avisa si `settings.json` o el directorio de sesiones quedaron legibles por otros usuarios; los archivos de sesión se guardan con permisos 0600; el historial de prompts se limita a 1000 entradas; `CLAW_BASH_TIMEOUT_MS` ajusta el timeout por defecto de la herramienta bash y `CLAW_BASH_MAX_OUTPUT_BYTES` su límite de salida (4KB–1MB); el spinner indica el modelo (y effort) que está pensando y el «Done» final incluye la duración del turno.
...
En los builds multiagente: `--timeout-secs N` ajusta el timeout por agente desde el REPL, `--parallel` se acota a 1–16, el build **falla rápido** si no hay credenciales de proveedor en el entorno (antes moría dentro del Director con un error confuso), y el resultado queda persistido en `docs/SUMMARY.md`. El build gate y el smoke test detectan **pnpm/yarn/bun por su lockfile** (antes siempre npm); el presupuesto de bundle se ajusta con `CLAW_PERF_BUDGET_KB` y el timeout del smoke test con `CLAW_SMOKE_TIMEOUT_SECS`; los modelos por rol admiten overrides `CLAW_MA_{SIMPLE,MEDIUM,COMPLEX,DIRECTOR,SUPERVISOR}_MODEL`; las escaladas de reintento quedan auditadas en `docs/SUPERVISION.md`; y el digest del repo reconoce más ecosistemas (pnpm, Docker, Makefile, Deno, Composer, Gemfile).

**Calidad de nivel superior** (ronda 14): el QA visual es ahora **multimodal** — los screenshots reales del smoke test viajan como imágenes al agente (hasta 3, cap 1.5MB cada una), que critica el render de verdad, no solo el DOM textual; el pipeline **aprende entre builds** (`.multiagent/lessons.md`: los issues del Supervisor se destilan y se inyectan en los prompts de los developers de builds futuros del mismo proyecto); `--max-cost <usd>` corta el build con **estado reanudable** al tocar el techo (aviso al 80%); con `CLAW_MA_FAILOVER=1`, tres errores 5xx consecutivos **cambian de proveedor a mitad de build** si tienes credenciales alternativas; el gate de diseño en `/improve` audita **solo los archivos que tocó el cambio**; y `docs/build-report.json` acompaña al SUMMARY con datos máquina-legibles (fases con gasto, tareas, hallazgos por gate, modelos efectivos). El sistema de prompts queda blindado por **evals E2E sobre los requests capturados** (verifican lo que de verdad se envió al API, rol por rol) y **21 golden files** (`tests/goldens/` — cambiar un prompt aparece en el diff del PR como prosa revisable; `CLAW_UPDATE_GOLDENS=1` los regenera). En el REPL: los agentes en background anuncian su final **encima de la línea que estás escribiendo** (ExternalPrinter), `/doctor fix` repara permisos laxos y el hooksPath, y `/design-review changed` (también `claw design-review --changed`) audita solo tus archivos cambiados. Los apoyos de ingeniería: **proptest** (19 propiedades sobre redacción de secretos, clamps de env, contraste y parsers), **cargo-deny**, tests en **Windows** y un **ratchet de cobertura** en CI.

**Sistema de prompts endurecido** (el corazón de `/web`/`/app`): el Director ahora entrega **copy real por página** (`page_content`: headline, subtítulo y CTAs publicables — la causa raíz del lorem ipsum, cortada en origen) y **non-goals vinculantes** (lo que NO se construye; el Subdirector tiene prohibido crear tareas para ello); los cuatro arquitectos reciben **briefs estructurados con 6 secciones obligatorias** cada uno (Backend: tabla endpoint×auth×DTO×errores; Frontend: árbol de rutas con data-fetching; Software: mapa de módulos y contratos; DevOps: matriz entorno×secretos) — la mejora que ya había transformado al diseñador, replicada; cada TaskSpec lleva un **test manual con pasos de click concretos** que el developer debe recorrer antes de reportar; el developer tiene **DoD ejecutable** (correr build/lint y pegar la salida antes de reportar éxito), prohibición explícita de dead-UI y **formato de reporte estructurado** (archivos/decisiones/fuera de alcance/build); el Supervisor recibe el **`git diff` real y el contenido de los archivos nuevos** en el prompt (revisa lo que cambió, no lo que quiera mirar) y cada issue debe **citar la línea exacta como evidencia** (registrada en SUPERVISION.md); los hallazgos que el gate determinista no logró arreglar **entran como input del QA visual** («son hechos, no opiniones»); Director y Subdirector llevan el contrato de salida JSON **al principio y al final** del prompt (los modelos pequeños pierden instrucciones de cola); y todos los prompts quedan **unificados en inglés técnico** con la regla de escribir el contenido de cara al usuario en el idioma del prompt original.

Ronda actual del multiagente: `--dry-run` en `/web`/`/app` **solo planifica** (Director → Arquitectos → Subdirector escriben `plan.json`/`backlog.json` y el presupuesto) sin desarrollar nada — perfecto para previsualizar alcance y coste; los builds greenfield interrumpidos se pueden **reanudar** (`--resume`) desde la última fase completada saltándose las tareas ya entregadas; el gasto se imprime **en vivo al cerrar cada fase** (delta + total acumulado); el design gate suma cinco auditorías nuevas — inputs sin `<label>`/aria-label, orden de headings roto, enlaces con texto genérico («click aquí», «read more»), `@font-face` sin `font-display`, y texto sobre gradientes (contraste no computable → aviso de verificación manual) — y `[PESO]` ahora también audita **fuentes** (.ttf/.otf sin .woff2 equivalente); y el E2E con mock cubre además una **ola de desarrollo completa** (scheduler → developer → delivery → SUMMARY) sin gastar tokens reales.

Novedades del pipeline multiagente: `--archetype <nombre>` fuerza el arquetipo de diseño cuando la detección por keywords no acierta (los 9 nombres válidos, case-insensitive; el error los lista); al drenarse el grafo de tareas hay un **pase final que reintenta una vez las tareas fallidas** y desbloquea a sus dependientes si el reintento triunfa (un segundo fallo es terminal — sin bucles); el gasto de tokens queda **atribuido por fase/rol** (Director, Arquitectos, Subdirector, Fundación, Desarrollo, Gates) con desglose al final del build y aviso auditado si un rol excede su cuota proporcional; el design gate añade la categoría **`[PESO]`** (imágenes de `public/`, `assets/`, `static/` por encima del presupuesto — `CLAW_IMG_BUDGET_KB`, 500KB por defecto); los design tokens emiten un bloque **`prefers-contrast: more`** con overrides endurecidos que pasan el propio gate de contraste; el formato de `docs/SUMMARY.md` está fijado por un golden test; y existe un **test E2E de la fase de planificación completa** (Director → Arquitectos → Subdirector) contra un mock del API con routing por contenido — cero tokens reales.
...
Más protecciones del REPL y del runtime: `@ruta<Tab>` **completa rutas del sistema de archivos** en cualquier punto del prompt (los directorios completan con `/` para seguir descendiendo; los ocultos solo aparecen si tecleas el punto); pegar **más de 50 líneas pide confirmación** antes de gastar tokens (Enter descarta, y el texto se recupera con ↑); si el proveedor se cae (5xx/sobrecarga/saldo) y tienes **otras credenciales configuradas**, la pista de error te lo recuerda; si tu credencial es un **JWT expirado o a <7 días de expirar** (tokens de Z.ai, por ejemplo), el arranque y `/provider show` lo avisan antes de que falle el primer turno; `CLAW_NET_TIMEOUT_MS` ajusta el timeout de red de los clientes API (5s–10min); los **secretos con prefijo conocido** (sk-ant-, ghp_, xoxb-, AKIA…) se enmascaran también al persistir la sesión en disco; y cuando `edit_file` no encuentra el bloque, el error incluye un **mini-diff del candidato más parecido** (esperado con `-`, encontrado con `+`) en vez de solo señalar una línea.

Para CI y mantenimiento: `claw design-review [--output-format json]` ejecuta el gate de diseño **sin sesión ni tokens** y sale con código 1 si hay hallazgos (perfecto como check de pipeline); `./install.sh --uninstall` (y `.\install.ps1 -Uninstall`) borran los binarios compilados con confirmación — el directorio de settings solo se borra con un sí interactivo explícito porque contiene tus claves; `scripts/bench-startup.sh` mide el arranque del binario contra un umbral (`CLAW_BENCH_MAX_MS`); y `.github/scripts/check_usage_commands.py` compara el registro de comandos con esta guía para que ningún comando nuevo quede sin documentar.

Honestidad del REPL y protecciones nuevas: `/help` marca automáticamente como **`(experimental)`** los comandos registrados que aún no tienen implementación (se detecta parseando el registro, así el marcado no se desactualiza); se purgó la lista interna que ocultaba del Tab, del help y del modo resume a más de 25 comandos ya implementados (incluido `/web`); `/diff` pinta el diff en color (verde/rojo/cian, respetando `NO_COLOR` y `/color off`); `/export` pasa por la misma **redacción de secretos** que la sesión en disco antes de escribir el transcript; si la sesión anterior murió sin cierre limpio (crash, terminal cerrada), el arranque te ofrece el `--resume` para retomarla; los 429/503 con header **`Retry-After`** esperan exactamente lo que pide el proveedor (con tope de 120s); `edit_file` avisa si el archivo **cambió en disco desde la última lectura**; `CLAW_MAX_SESSION_MB` avisa (una sola vez) cuando la sesión en disco crece de más; y `CLAW_READ_FILE_MAX_BYTES` corta las lecturas gigantes con un error claro que sugiere offset/limit (las ventanas sobre archivos grandes se leen en streaming, sin cargar el archivo entero).
