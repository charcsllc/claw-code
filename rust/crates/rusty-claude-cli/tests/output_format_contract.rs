...
fn direct_resume_safe_slash_commands_route_to_local_json_actions_831() {
    ...
    // `/sandbox` status depends on the host (namespace/filesystem sandbox
    // support varies per machine), so it only asserts a valid value; the
    // #831 contract under test is the routing, not the sandbox state.
    for (command, expected_kind, expected_status) in [
        ("/version", "version", Some("ok")),
        ("/sandbox", "sandbox", None),
        ("/diff", "diff", Some("ok")),
        ("/status", "status", Some("ok")),
    ] {
        ...
        match expected_status {
            Some(expected) => assert_eq!(parsed["status"], expected, "{command} status: {parsed}"),
            None => assert!(
                ["ok", "warn", "error"]
                    .iter()
                    .any(|valid| parsed["status"] == *valid),
                "{command} status must be ok/warn/error: {parsed}"
            ),
        }
        assert_ne!(
